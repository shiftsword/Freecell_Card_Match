...
class FreecellOCRApp:
    def __init__(self, root):
        ...
        # 创建界面组件
        self.create_widgets()
        self.setup_directories(["Single_Card_Images", "Card_Rank_Images"], clear=True)
        self.load_template_sets()

    def setup_directories(self, directories, clear=False):
        """检查并创建目录，可以选择清空目录"""
    ...
    def load_template_sets(self):
        ...
        if len(template_sets) == 1:  # 只有 "自动" 选项
            self.template_set.set("自动")
        else:
            self.template_set.set("自动")  # 默认选择自动
    ...
    def run_processing(self, image_path):
        ...
        try:
            ...
            sys.stdout = lambda message: self.result_text.after(0, lambda m=message: (self.result_text.insert(tk.END, m), self.result_text.see(tk.END)))
            ...
            try:
                ...
                # 在识别开始前创建日志文件
                log_file = self.open_log_file("Card_Match_Result.log")
    ...
    def clear_image_directories(self):
        """清空图像处理目录"""
        self.manage_image_directories(clear=True)
    ...
    def manage_image_directories(self, clear=False):
        """检查并创建必要的目录，可以选择清空目录"""
